    ...
    def parse_user(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')) and \
            ...
            for user_info in user_infos:
                ...
                user_item['description'] = user_info.get('desc1') if ('粉丝' not in user_info.get('desc1')) else '暂无简介'
                user_item['follows_count'] = user_info.get('user').get('follow_count')
                user_item['fans_count'] = user_info.get('user').get('followers_count')
                user_item['cover'] = user_info.get('user').get('profile_image_url').split('?')[0]
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')):
            ...
            for weibo_info in weibo_infos:
                ...
                if mblog:
                    ...
                    weibo_item['raw_text'] = mblog.get('raw_text') if mblog.get('raw_text') else '原创微博'
                    weibo_item['pictures'] = mblog.get('pics')
                    weibo_item['source'] = mblog.get('source') if mblog.get('source') else '来源未指定'
